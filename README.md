...
Что делать, если файл с вопросами пуст? Может ли быть так, что на основе данных существует равная вероятность того, что результат будет человеком или репликантом?

Ваша тестовая реализация скорее всего состоит из нескольких функций и, предположительно, классов. Ваша цель — охватить тестами все крайние случаи для всех компонентов. В принципе, всякий раз, когда оператор теста вводит что-то неправильное (например, выбирая несуществующий ответ или выходящие за пределы допустимого значения для измерений, например, отрицательную частоту сердечных сокращений), он или она должен получать осмысленное информационное сообщение и возможность повторить ввод.

Во время этого упражнения вы, скорее всего, перепишете хотя бы часть кода из EX00, но в этом и весь смысл. Кроме того, при написании тестов настоятельно рекомендуется использовать фреймворк [Pytest](<https://realpython.com/pytest-python-testing/>). Все тесты должны быть внутри `tests` каталога.
...
- [Автоматически сгенерированная](<https://www.sphinx-doc.org/en/master/usage/extensions/autodoc.html>) ссылка на код

В первой части вы можете использовать Markdown или RST для текста и форматирования. Во второй части вам нужно будет добавить [комментарии](<https://realpython.com/documenting-python-code/>) ко всем объектам в вашем коде — модулям, функциям, классам и т. д.
